...
graph = create_nodes()
a_star_connect_nodes(graph)

def huristic_function(neighbor):
		return (neighbor.distance / neighbor.speed_limit)
...
def astar(start, goal):
	...
	if(start_node.nodeId == goal):
		a_star_solution(start_node, start, visited_num)
	...
	while frontier:
		...
		if(node.nodeId == goal):
			cost = node.cost
			a_star_solution(node, start, visited_num)
			break
		...
		for neighbor in node.neighbors:
			...
			if(not neighbor.node.visited):
				...
				visited_num = visited_num + 1
				if(neighbor.node.cost == 0 or (huristic_function(neighbor) + node.cost)<neighbor.node.cost):
...
astar(str(5930756061), str(5843293191))
